...
static const uint32_t PRIME_ID = 4294967291; // MAXPRIME[32];
static const uint32_t PRIME_FING = 4294967291; // MAXPRIME[32];
...
public:
    ...
    Fermat(int _a, int _e, bool _fing) : array_num(_a), entry_num(_e), use_fing(_fing), fingerprint(nullptr)
    {
        create_array();
    }

    Fermat(int _memory, bool _fing) : use_fing(_fing), fingerprint(nullptr)
    {
        array_num = 3;
        if (use_fing)
    ...
    void Insert(uint32_t flow_id, uint32_t cnt)
    {
        if (use_fing)
        {
            uint32_t fing = MurmurHash3_x86_32((char *)&flow_id, sizeof(uint32_t), 100);
            for (int i = 0; i < array_num; ++i)
            {
                uint32_t pos = MurmurHash3_x86_32((char *)&flow_id, sizeof(uint32_t), i) % entry_num;
    ...
    void Insert_one(uint32_t flow_id)
    {
        ...
        if (use_fing)
        {
            uint32_t fing = MurmurHash3_x86_32((char *)&flow_id, sizeof(uint32_t), 100) % PRIME_FING;
            for (int i = 0; i < array_num; ++i)
            {
                uint32_t pos = MurmurHash3_x86_32((char *)&flow_id, sizeof(uint32_t), i) % entry_num;
    ...
    bool verify(int row, int col, uint32_t &flow_id, uint32_t &fing)
    {
        ...
        if (!(MurmurHash3_x86_32((char *)&flow_id, sizeof(uint32_t), row) % entry_num == col))
            return false;
        if (use_fing && !(MurmurHash3_x86_32((char *)&flow_id, sizeof(uint32_t), 100) % PRIME_FING == fing))
            return false;
        return true;
    }
            ...
            for (int j = 0; j < entry_num; ++j)
            {
                ...
                else if (verify(i, j, flow_id, fing))
                {
                    ...
                    for (int t = 0; t < array_num; ++t)
                    {
                        if (t == i)
                            continue;
                        uint32_t pos = MurmurHash3_x86_32((char *)&flow_id, sizeof(uint32_t), t) % entry_num;
